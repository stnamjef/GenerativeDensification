...
def main(cfg):
    ...
    names, depth_accs = [], []
    psnrs,ssims,psnrs_coarse, lpips_vggs, lpips_alexs = [],[],[],[],[]
    ...
    for i in tqdm(range(len(loader))):#len(loader)
        ...
        #     # print('all_nums:', _opacity.shape)
        #     # print('nums:', _centers.shape)
        #     # import pdb; pdb.set_trace()
        #     dtype = [('x', 'f4'), ('y', 'f4'), ('z', 'f4'),
        #             ('red', 'u1'), ('green', 'u1'), ('blue', 'u1')]
        #     xyz = _centers.squeeze().cpu().numpy()
        #     # normals = pc[1].squeeze()
        #     # rgb = _shs.squeeze() # * 255.0
        #     rgb = np.full(xyz.shape, [255, 0, 0])
        #     elements = np.empty(xyz.shape[0], dtype=dtype)
        #     attributes = np.concatenate((xyz, rgb), axis=1)
        #     elements[:] = list(map(tuple, attributes))
        #     # print('xyz', xyz.max(), xyz.min()) 
        #     # Create the PlyData object and write to file
        #     vertex_element = PlyElement.describe(elements, 'vertex')
        #     ply_data = PlyData([vertex_element])
        #     ply_data.write(os.path.join(cfg.infer.save_folder, name + '.ply'))
        # ################################

        if i<100:
            # cv2.imwrite(os.path.join(cfg.infer.save_folder, name + '.jpg'), torch.cat((img_gt,images),dim=0).detach().cpu().numpy()[...,::-1]*255)
            cv2.imwrite(os.path.join(cfg.infer.save_folder, name + '.jpg'), torch.cat((img_gt,images, images_coarse),dim=0).detach().cpu().numpy()[...,::-1]*255)
        ...
        if cfg.infer.eval_novel_view_only:
            ...
            images_coarse = images_coarse.permute(2,0,1)[None][...,width*n_view:]
        else:
        ...
        if images.shape[-1] > 0:
            ...
            color_loss_all_coarse = (images_coarse-img_gt)**2
            psnr = -10. * torch.log(color_loss_all.mean()) / torch.log(torch.tensor([10.]).to(device))
            psnr_coarse = -10. * torch.log(color_loss_all_coarse.mean()) / torch.log(torch.tensor([10.]).to(device))

            if psnr_coarse > psnr:
                # print("psnr:", psnr)
                # print("psnr_coarse", psnr_coarse)
                psnr = psnr_coarse
                # print("img_name:", os.path.join(cfg.infer.save_folder, name + '.jpg'))
            ...
            psnrs_coarse.append(psnr_coarse.item())
        ...
        if cfg.infer.video_frames > 0:
            ...
            for cam in cams:
                ...
                output_img = my_system.net.gs_render.render_img(cam, rays, _centers, _shs, _opacity, _scaling, _rotation, device)
                # import pdb; pdb.set_trace()
                # img, normal = output_img['image'], output_img['rend_normal']
                img = output_img['image']
    ...
    if len(psnrs) and cfg.infer.metric_path is not None:
        print(f'evaluation score, psnr: {np.mean(psnrs)} ssim: {np.mean(ssims)}, psnr_coarse: {np.mean(psnrs_coarse)}, lpips_vgg:{np.mean(lpips_vggs)}, lpips_alex: {np.mean(lpips_alexs)}, depth_acc:{mean_depth_acc}')
