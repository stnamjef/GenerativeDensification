...
class GaussianAdapter(nn.Module):
    ...
    def forward(
        ...
        # print('depths_r:', depths.max(), depths.min())

        # ##### Visualization 3DGS #####
        # def save_ply(means, sh, opacities, scales, rotations):
        #     import numpy as np
        #     from plyfile import PlyData, PlyElement
        #     V, N, _ = means.squeeze().shape
        #     _centers = means.squeeze().reshape(V*N,3)
        #     _shs = sh.squeeze().reshape(V*N,3,25).permute(0,2,1)
        #     _opacity = opacities.squeeze().reshape(V*N).unsqueeze(dim=1)
        #     _scaling = scales.squeeze().reshape(V*N,3)
        #     _rotation = rotations.squeeze().reshape(V*N,4)           
        #     _features_dc = _shs[:, 0:1, :]
        #     _features_rest = _shs[:, 1:, :]
        ...
        #     xyz = _centers.cpu().numpy()
        #     normals = np.zeros_like(xyz)
        #     f_dc = _features_dc.transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        #     f_rest = _features_rest.transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        ...
        # save_ply(means, sh, opacities, scales, rotations)
