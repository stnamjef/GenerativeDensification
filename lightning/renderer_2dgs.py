...
class Renderer(nn.Module):
    ...
    def render_img(
        ...
        if scales is not None:
             scales = self.get_scaling(scales)
        ...
        if screenspace_points == None:
            screenspace_points = torch.zeros((centers.shape[0], 4), dtype=centers.dtype, requires_grad=True, device=device) + 0
            # screenspace_points = (
            #     torch.zeros_like(
            #         centers.shape[0], 4,
            #         dtype=centers.dtype,
        ...
        return {
            ...
            # f"viewspace_points{prex}": screenspace_points,
