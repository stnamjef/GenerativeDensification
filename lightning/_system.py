...
from lightning._network_nst import Network
...
class system(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # if len(output['render_pkg']) > 1:
        #     _centers_coarse, _shs_coarse, _opacity_coarse, _scaling_coarse, _rotation_coarse = output['render_pkg'][0]
        #     _centers_fine, _shs_fine, _opacity_fine, _scaling_fine, _rotation_fine, mask = output['render_pkg'][1]
        #     self.log('_nums_coarse', _centers_coarse[mask].shape[0])
        #     self.log('_opa_mean_coarse', torch.sigmoid(_opacity_coarse[mask]).mean())
        #     self.log('_sca_mean_coarse', torch.exp(_scaling_coarse[mask]).mean())
        #     self.log('_nums_fine', _centers_fine.shape[0])
        #     self.log('_opa_mean_fine', torch.sigmoid(_opacity_fine).mean())
        #     self.log('_sca_mean_fine', torch.exp(_scaling_fine).mean())
